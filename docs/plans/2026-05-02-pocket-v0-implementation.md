...
6. **Reuse Phase 3 thresholds.** Whatever `worker/config/thresholds.json` lands at end of Phase 3 is what Pocket v0 uses. No new bench eval required for v0.
...
1. **Filter threshold — same as match threshold or slightly looser?** Phase 3 sets a strict `match` threshold for high precision. Pocket's "You" feed may want higher recall (it's OK to show a near-match; the user can dismiss). Decide after running the filter on `bench/fixtures/group_shot/`.
2. **Contributor IP rate limit value.** v0 ships with *some* rate limit on contributor uploads to defeat trivial abuse. Number TBD — start with 100 photos / IP / hour, revisit if real users complain.
3. **What happens if the contributor uploads a photo with no detected faces?** Two options: (a) accept and store anyway (it might be a venue shot the owner wants), (b) reject with "no faces found" message. v0 default: accept. Surface only in "All contributions" view (not in "You" feed). The "All" view is itself a v1 feature; v0 just stores them.
4. **Token URL format.** `/p/{token}` or `/p/{eventId}/{token}`? Former is shorter and leaks no event ID; latter is debuggable. Recommend former.
5. **iOS Safari upload size limit.** iOS imposes ~50MB per upload via web. Need to test multi-photo selection of large iPhone HEIC photos. May need client-side resize before PUT.
...
- `verifyShareLink(token)` — looks up by hash, checks not revoked / not expired / under max_uploads. Returns `{eventId}` or throws.
- `recordContribution(token)` — atomic increment of `upload_count`.
...
**Files:** `app/api/p/[token]/upload/route.ts`, `tests/integration/contributor-upload.test.ts`

POST endpoint:
- Verify token via `verifyShareLink`.
- Mint a signed R2 PUT URL via existing `createPresignedPutUrl`.
- Return `{uploadUrl, photoId}`.

POST `/api/p/[token]/finalize/[photoId]`:
- Mark photo `ready`, set `uploader_token = token`, enqueue `photo_jobs` row.
- Increment `share_links.upload_count`.

**Done when:** integration test uploads 3 photos via this flow, asserts photos exist with `uploader_token` set and `uploader_user_id NULL`, and a `photo_jobs` row exists per photo.
...
- If detection returns ≥ 1 face with `confidence > 0.9`, takes the highest-confidence face's embedding.
- Writes to `users.face_embedding`, `users.face_quality_score`, `users.face_enrolled_at`.
- Returns `{quality, faceCount}`.

**Done when:** integration test posts a selfie fixture and asserts the user row gets an embedding.
...
- Query: for each `photo` in this event with `processing_state='ready'`, find its `face_detections`, compute `embedding <=> $owner_embedding`, return photo if any detection's distance < threshold.
- Returns `[{photoId, r2KeyPreview, distance, takenAt}]` ordered by `takenAt DESC`.
...
Assertion: returned photos exactly equal the labeled set for that identity. Zero false positives, ≥ 80% recall.

**Done when:** test passes with current Phase 3 thresholds.
...
**Files:** `e2e/pocket-owner.spec.ts`, `e2e/pocket-contributor.spec.ts`
...
**Done when:** both specs pass in CI.
