...
// TODO: per-IP rate limit (Pocket v1)

// 15 min — matches the PUT URL TTL. Contributors don't pause mid-upload like owners do,
// and a tighter window limits cap-pinning abuse from a single share link.
...
export async function POST(req: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  // Event-existence + expiry check, cap-count + INSERT in a single TX.
  // No membership check — contributors aren't members.
    ...
    .transaction(async (tx) => {
      ...
      const [row] = await tx
        ...
        .values({
          eventId,
          uploaderToken: token,
          // uploaderUserId intentionally omitted — XOR check requires exactly one of the two
